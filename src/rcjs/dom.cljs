(ns ^:figwheel-always rcjs.dom
    (:require [cljsjs.react])
    (:require-macros [rcjs.reactjure :refer [deftags]]))

(deftags a abbr acronym address applet area article aside audio b base basefont 
        ...
        html i iframe image img input ins isindex kbd keygen label legend li link 
        listing main map mark marquee menu menuitem meta meter multicol nav 
        nobr noembed noframes noscript object ol optgroup option output p 
        param picture plaintext pre progress q rp rt rtc ruby s samp script 
        section select shadow small source spacer span strike strong style 
